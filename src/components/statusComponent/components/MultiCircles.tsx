...
const MultiArcCircle = ({
  ...
  const paths = intervals.map((angles) => {
    ...
    const {
      start: startPointInternal,
      end: endPointInternal,
      middle: middlePointInternal,
    } = getPointsByDescriptor(angles, center, internalRadius);

    return angles.end - angles.start <= 180 ? (
      <Path
        key={`${angles.start}.${angles.end}`}
      ...
      <Path
        key={`${angles.start}.${angles.end}`}
        d={`M ${startPoint.x} ${startPoint.y} 
        A ${radius} ${radius} , 0, 0, 1, ${middlePoint && middlePoint.x} ${
          middlePoint && middlePoint.y
        }
        A ${radius} ${radius} , 0, 0, 1, ${endPoint.x} ${endPoint.y}
        A ${EDGE_RADIUS} ${EDGE_RADIUS} , 0, 0, 1, ${endPointInternal.x} ${
          endPointInternal.y
        }
        A ${internalRadius} ${internalRadius} , 0, 0, 0, ${
          middlePointInternal && middlePointInternal.x
        } ${middlePointInternal && middlePointInternal.y}
        A ${internalRadius} ${internalRadius} , 0, 0, 0, ${
          startPointInternal.x
        } ${startPointInternal.y}
        A ${EDGE_RADIUS} ${EDGE_RADIUS} , 0, 0, 1, ${startPoint.x} ${
          startPoint.y
        }
      `}
        fill={color}
  ...
  const {start: startPoint, end: endPoint} = getPointsByDescriptor(
    {start: 0, end: 180},
    center,
    radius,
  );
